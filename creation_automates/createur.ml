...
let soigneur1 = creerAutomate [errant 1 0 1; errant 1 1 0; medecin 10 0 1; medecin 10 1 0; constructeur 1 piege 118 1 0; (List.map (fun d -> [(e1,Et(Libre(d),OrdreDonne),piege d,0,100)]) [N;S;E;O]); (List.map (fun d -> [(1,Et(Libre(d),OrdreDonne),piege d,0,100)]) [N;S;E;O])]
...
let soigneur2 = creerAutomate [errant 1 0 1; errant 1 1 0; medecin 10 0 1; medecin 10 1 0; constructeur 1 piege 124 1 0; constructeur 1 piege 124 0 1;; (List.map (fun d -> [(e1,Et(Libre(d),OrdreDonne),piege d,0,100)]) [N;S;E;O]); (List.map (fun d -> [(1,Et(Libre(d),OrdreDonne),piege d,0,100)]) [N;S;E;O])]
